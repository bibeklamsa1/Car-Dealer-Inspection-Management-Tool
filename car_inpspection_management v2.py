...
def add_car():
    ...
    cursor = conn.cursor()
    cursor.execute(''' 
        INSERT INTO cars (dealer, car_model,mr_number, purchase_date, inspection_date)
        VALUES (?, ?, ?,? , ?)
    ''', (dealer, car_model,mr_number, purchase_date, inspection_date))
    conn.commit()
    conn.close()
...
def update_table():
    ...
    for car in cars:
        inspection_date = datetime.strptime(car[5], "%Y-%m-%d")
...
def apply_filter():
    ...
    for car in filtered_data:
        ...
        elif (current_date <= inspection_date <= expire_date):
            table.insert("", "end", values=(car[0], car[1], car[2], car[3], car[4],car[5], remaining_days), tags=("expiring",))
...
columns = ("ID", "Dealer Name", "Car Model", "MR Number", "Purchase Date", "Last Date For Inspection", "Days Remaining to Expire")
...
tk.Label(frame, text="Last Date For Inspection:", font=("Helvetica", 12)).grid(row=4, column=0, padx=10, pady=5)
inspection_date_entry = DateEntry(frame, font=("Helvetica", 12), width=40, date_pattern="dd-mm-yyyy")
inspection_date_entry.grid(row=4, column=1, padx=10, pady=5)
